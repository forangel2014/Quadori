...
class LLM():
    ...
    def __init__(self, model, device=None):
        ...
        if model == 'gpt-j':
            ...
            self.local_lm = AutoModelForCausalLM.from_pretrained(model_path,device_map=self.device,torch_dtype=torch.float16,load_in_8bit=True)
    ...
    def __call__(self, prompt, stop=["\n"], temperature=0):
        if self.model == 'gpt3':
            ...
            response = self.chatgpt(prompt, stop)
        elif self.model == 'gpt4':
            response = self.gpt4(prompt, stop)
        elif self.model == 'gpt-4o-mini':
            response = self.gpt4omini(prompt, stop)
        else:
    ...
    def gpt4omini(self, prompt, stop=["\n"], temperature=0):
        response = openai.ChatCompletion.create(
                model="gpt-4o-mini", 
                messages=prompt,
                temperature=0,
